...
// Each candidate joins the same feedback arrow. One bead makes the return trip.
export function createFeedbackLoop(){
  ...
  const cards=[],paths=[],branches=[],routes=[];
  ...
  const feedbackPulse=new THREE.Mesh(new THREE.SphereGeometry(.038,20,14),pulseMaterial);
  feedbackPulse.name='feedback-pulse';group.add(feedbackPulse);
  const haloMaterial=pulseMaterial.clone();
  const halo=new THREE.Mesh(new THREE.SphereGeometry(.065,16,12),haloMaterial);feedbackPulse.add(halo);
  ...
  function setTime(seconds){
    ...
    const selection=smooth(p/.07)*(1-smooth((p-.91)/.09));
    const arrival=smooth((p-.88)/.07)*(1-smooth((p-.96)/.04));
    const travel=clamp((p-.025)/.91);
    ...
    cards.forEach((card,i)=>{
      ...
      card.material.color.copy(silverColor).lerp(accentColor,selected?selection*.65:0);
      card.material.opacity=selected?1:1-selection*.24;
      paths[i].material.color.copy(lineColor).lerp(accentColor,selected?selection*.35:0);
      paths[i].material.opacity=selected?.38:.23;
    ...
    pulseMaterial.opacity=smooth(p/.025)*(1-smooth((p-.965)/.035));
    haloMaterial.opacity=pulseMaterial.opacity*.14;
    arrowMaterial.opacity=.65+arrival*.35;
    const phase=p>.95?'generate':p<.16?'evaluate':p<.82?'feedback':'improve';
    return {candidate,phase,travel,cycle};
  }
  function setTheme(isDark){
    ...
    pulseMaterial.color.copy(accentColor);haloMaterial.color.copy(accentColor);arrowMaterial.color.copy(accentColor);
